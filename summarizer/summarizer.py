...
class Summarizer(object):
    def __init__(self, articles, sentence_tokenizer, features_extractor,
        ...
        self.model_summary_path = 'model_summaries'
        self.system_summary_path = 'system_summaries'

    def train(self):
        features = []
        output = []

        for a in self.articles[:4]:
    ...
    def test_article(self, article, article_id):
        for i, s in enumerate(article.summaries):
            with open(os.path.join(self.model_summary_path, "model_summary_{}_{}.txt".format(article_id, i)), "w") as text_file:
                sents = self.sentence_tokenizer.tokenize(unidecode(s))
                text_file.write('\n'.join(sents))
        model_summary = self.predict(article.text)
        with open(os.path.join(self.system_summary_path, "system_summary_{}.txt".format(article_id)), "w") as text_file:
